...
#create jags arguments and save as RData files
#adapted from D.Morin code 20190617_formatting example function
#adapted from SCR_HR_2020-07-06 to estimate sigma and lambda from data from all grids instead of each grid separately
...
#so, only expect to catch PEMA in the small trap grid, so need to specify that reduced trap file (P-Y)
...
#data2oscr(edf=rbs.edf, sess.col=1, id.col=2, occ.col=4, trap.col=3, K=c(7,5,6,4), tdf=list(rbs.tdf,rbs.tdf,rbs.tdf,rbs.tdf), ntraps=rep(50,4))
...
SCR_B.fun<-function(data, n.chains, n.adapt, n.iter, n.burnin, n.cores, sp.cr){
  ...
  data.SCR_B <- list(n.sess=length(data$sf$caphist), mmax=mmax, J=ncol(data$sf$caphist[[1]]),
                     ...
                     S.Area=S.Area)
  #initial values
  inits <- function(){list(lam0=runif(1), psim=runif(length(data$sf$caphist)),
  ...
  params <- c("lam0","sigma","psim", "N", "D", "zm", "S")
  
  #save as .RData
  save(data, data.SCR_B, inits, params, n.chains, n.adapt, n.iter, n.burnin, sp.cr, file=paste("RData/CR-SCR/SCR_allgrids_jagsargs_", sp.cr, ".RData", sep=""))
}
...
p <- function(sp.cr, traps) #g = grid, sp.cr = species name
{
  ...
  print(table(data.cr[,c("Cnum","Rlet")]))
  print(table(traps[,c("Cnum","Rlet")]))
  ...
  else
  {
    ...
    write.table(paste(sp.cr, "formatted grid for SCR ", Sys.Date()), "test.txt", sep=",", append=T, col.names=F)

    # oSCR <- oSCR.fun(data=cr)
    # out <- capture.output(oSCR)
    # cat(out, file=paste("Output_oSCR/", sp.cr, "_oSCR_", Sys.Date(), ".txt", sep=""), sep="\n", append=F)
    # 
    # pred.df <- data.frame(session=factor(1:9))
    # reals <- get.real(model=oSCR$mod, type="dens", newdata=pred.df)
    # reals[,1:4] <- reals[,1:4]*10000 #make n per ha
    # write.table(reals, file=paste("Output_oSCR/", sp.cr, "_allgrids_oSCR_", Sys.Date(), ".txt", sep=""))
    # 
    # ggplot(data=reals, aes(x=estimate*10000, y=session)) + 
    #   geom_point() + 
    #   geom_errorbar(aes(xmin=lwr*10000, xmax=upr*10000), width=0.1) +
    #   theme_bw()
    
    # gr <- get.real(oSCR$mod, type="dens") #type can also be "dens", "det", "sig", "all"
    # gr <- gr[[1]]
    # 
    # ggplot(data=gr) + geom_point(aes(x=X, y=Y, col=estimate)) + theme_bw(base_size=15) + coord_equal()
